...
def TCS(total_length, t, T_total, C, V_0, alpha, out_rate, iter_time = 25, fpr = 0.01):
    final_prvl_rate = []
    ...
    for iterative in range(iter_time):
        ...
        Initial_infected_num = CRN(T_total, V_0)
        print("iteration ", iterative)
        ERROR, flag = 0, 0
        # Initialization
        T = T_total # Total population
...
#                print(Num_test_today)
                # Switch formulations: use total number or per person
...
#                print(len(Sus_Individuals))
                new_infected_num = np.random.binomial((TI_test[i] + NTI_test[i]), alpha)+np.random.binomial(len(Sus_Individuals), out_rate)  # include outside contact
...
#    day_vector = range(total_length+1)
    ...
    return final_prvl_rate_estimated, average_size, average_test_times, average_RQ, average_WQ, average_fn

#x1,x2,x3,x4,x5,x6 = OTCL(14,3,10000,300,0.001,0.2,0.0005)
#print(x1)
#print(x2)
